...
function ProcessNewPosition(x, y, z)
    ...
    if (IsInputDisabled(0) and not charControl) then
        if (IsDisabledControlPressed(1, Cfg.controls.keyboard.pointPlayer)) then
            PointCamToPlayer()
            print('111')
        end
        if (IsDisabledControlPressed(1, Cfg.controls.keyboard.stopPointPlayer)) then
            StopPointCamToPlayer()
        end
...
function PointCamToPlayer()
    local playerPed = GetPlayerPed(-1)
    local playerCoords = GetEntityCoords(playerPed)
    PointCamAtCoord(cam, playerCoords.x, playerCoords.y, playerCoords.z)
    --PointCamAtEntity(cam, playerPed, 0.0, 0.0, 0.0, 1)
    print('111')
end

function StopPointCamToPlayer()
    phase = GetCamAnimCurrentPhase(cam)
    StopCamPointing(cam)
